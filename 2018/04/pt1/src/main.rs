...
fn main() {
            ...
            |(ts, msg)| {
                match msg {
                    ...
                    Log::Action(action) => match action {
                        ...
                        Action::Wake => {
                            let sleep_time = sleep_time.take();

                            if let (Some(guard), Some(sleep_time)) = (guard, sleep_time) {
    ...
    let (max_guard, max_minutes, _) = guard_sleep_times.iter()
        .map(
            |(guard, sleep_times)| sleep_times.iter()
                .fold((guard, [0; 60], 0), |(guard, mut minutes, mut total), (sleep_time, &wake_time)| {
    ...
    let max_minute = max_minutes.iter().enumerate()
        .max_by_key(|(_, m)| *m)
        .unwrap()
        .0 as i32;
    ...
    println!("{} at {} -> {}", max_guard, max_minute, max_guard * max_minute);
}
